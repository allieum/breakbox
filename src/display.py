...
def on_param_changed(param: Param, name: str):
    def on_change(value):
        global last_changed_param
        fullness = param.normalize(value)
        last_changed_param = ParamUpdate(name, value, fullness, time.time())
        logger.info(f"updated param {last_changed_param}")
        # draw bar / name / value
    return on_change
...
# sample states
prev_sample_states = None
def update(bpm, sample_states: list[SampleState]):
    global prev_sample_states
    if oled is None:
        return

    global last_refresh, last_text
